...
def _resolve_concept(concept: AnnotatedConcept) -> AnnotatedConcept:
    ...
    title = concept.title.strip() or slug
    return AnnotatedConcept(slug=slug, title=title, gloss=concept.gloss.strip())


def build_annotation(
    doc_id: str,
    section_id: str,
    concepts: Iterable[AnnotatedConcept],
    *,
    summary: str = "",
    model: str | None = None,
    ...
    validates it by membership against the document's sections. Concepts are
    deduplicated by slug: the first occurrence wins its title, and the first non-empty
    gloss for a slug wins.
    """
...
def merge_section_concepts(
    ...
    """Merge Tier-1 (auto) and Tier-2 (agent) concept edges, presence-based.

    For each section: if an annotation exists it is authoritative — its concepts
    become the section's edges (``source="agent"``), and an empty annotation therefore
    yields no edges (the false-positive prune); otherwise the deterministic extraction
    supplies the edges (``source="auto"``). Branching on annotation *presence* (never
    on whether its concept list is truthy) is what lets the empty list zero out a
    section's rows.
    """

    auto_by_section: dict[str, list[tuple[str, str]]] = {}
    for entry in extract_concepts(sections):
